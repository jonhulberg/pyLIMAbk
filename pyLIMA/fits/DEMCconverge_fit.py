...
class MCMCfit(MLfit):
    ...
    Checks for convergence and ends run when N >=50*tau
    Saves to hdf5
    Attempt to use DEMoves to shorten auto-correlation time
    ...
    def resume_fit(self,computational_pool=False):
        self.priors = parameters_priors.default_parameters_priors(
            self.priors_parameters)
        start_time = python_time.time()
        if computational_pool:
        ...
        #Read backend file from run.
        new_backend = emcee.backends.HDFBackend(self.hdf5path)
        iteration = new_backend.iteration
        nlinks = self.MCMC_links - iteration # remaining links to run ...
        nwalkers = new_backend.shape[0]
        number_of_parameters = new_backend.shape[1]
        if pool:
            with pool:
                sampler = emcee.EnsembleSampler(nwalkers, number_of_parameters,
                                                self.objective_function, pool=pool,backend=new_backend)

                sampler.run_mcmc(None, nlinks, progress=True)
        else:
            sampler = emcee.EnsembleSampler(nwalkers, number_of_parameters,
                                            self.objective_function, pool=pool,backend=new_backend)
            sampler.run_mcmc(None, nlinks, progress=True)
